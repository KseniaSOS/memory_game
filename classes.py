...
from helper_functions import create_card_frame, create_empty_frame, merge_str_line_by_line, create_nested_list
...
class BoardElement:
    ...
    def __init__(self, element_name: str, element_content: str, frame_elements: dict, frame_size: dict):
        ...
        self._card_front = create_card_frame(element_content, frame_elements, frame_size)
        self._card_back = create_card_frame(create_empty_frame(frame_size), frame_elements, frame_size)
    ...
    def __copy__(self):
        return BoardElement(self._element_name, self._element_content, self._frame_elements, self._frame_size)
...
class Board:
    ...
    def __str__(self):
        ...
        for row in range(0, self._board_size[0]):
            ...
            for column in range(1, self._board_size[1]):
                row_string = merge_str_line_by_line(row_string, self._board_content[row][column].__str__())
    ...
    def count_flipped_cards(self):
        ...
        for row in range(0, self._board_size[0]):
            for column in range(0, self._board_size[1]):
                number_flipped_cards += int(self._board_content[row][column].is_visible)
